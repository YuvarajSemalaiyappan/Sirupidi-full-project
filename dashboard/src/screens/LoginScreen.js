...
import { loginWithOTP, sendOtp, resendOtp} from "./../Redux/Actions/userActions";
...
const Login = ({ history }) => {
  ...
  const { error, loading, userAInfo, isOtpSent, isOtpResend } = userLogin;
  ...
  const sendOtpHandler = async (e) => {
    e.preventDefault();
    if (validateEmail()) {
      const isOtpSentSuccessfully = await dispatch(sendOtp(email));
